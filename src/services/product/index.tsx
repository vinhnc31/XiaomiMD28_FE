...
export default class ProductService extends BaseService<ProductModel> {
  ...
  public async getProductByColor(color: string) {
    const {data} = await http.get<ProductModel[]>(`${url.getProduct}/filter/Color?color=${encodeURIComponent(color)}`);
    return data;
  }


  public async getProductByConfig(config: string) {
    const encodedConfig = encodeURIComponent(config);
    const { data } = await http.get<ProductModel[]>(`${url.getProduct}/filter/Config?config=${config}`);
    return data;
  }
